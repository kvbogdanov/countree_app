...
class ViewPageState extends State<ViewPage> {
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        if (responseJson.containsKey('data')) {
          ...
          if (responseJson['data']['diameter'] == null)
            diameter = 0;
          else
            diameter = responseJson['data']['diameter'];
          ...
          if (responseJson['data']['overall'] != null && responseJson['data']['overall'] != 0) {
            if (responseJson['data']['overall'] == 1) overall = 'хорошее';
            if (responseJson['data']['overall'] == 2) overall = 'удовл.';
            if (responseJson['data']['overall'] == 3) overall = 'неудовл.';
          }
              ...
              Padding(
                  ...
                  child: Column(children: [
                    ...
                    conditionsWidgets.isNotEmpty
                        ? Row(
                            children: <Widget>[Column(crossAxisAlignment: CrossAxisAlignment.start, children: conditionsWidgets)],
                          )
                        : Container(),
                    surround.isNotEmpty
                        ? Row(
                            children: <Widget>[
                              Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: <Widget>[Text('Условия роста:', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)), Text('$surround')],
                              )
                            ],
                          )
                        : Container(),
                    neighboursWidgets.isNotEmpty
                        ? Row(
                            children: <Widget>[Column(crossAxisAlignment: CrossAxisAlignment.start, children: neighboursWidgets)],
                          )
                        : Container(),
                    SizedBox(height: 10),
                    Row(
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        body: SingleChildScrollView(child: treeinfoWidget(args.toString())));
  }
}
